...
namespace Drupal\file_resup\Plugin\Field\FieldWidget;
...
/**
 * Plugin implementation of the 'file_resup' widget.
 *
 * @FieldWidget(
 *   id = "file_resup",
 *   label = @Translation("File Resumable Upload"),
 *   field_types = {
 *     "file",
 *     "image"
 *   }
 * )
 */
class FileResupWidget extends WidgetBase implements ContainerFactoryPluginInterface {
...
//
//    kint("settings form");
//    kint($this->getSetting("resup_settings"));
...
//      '#weight' => 20,
    ...
    $element['resup_settings']['resup'] = array(
      ...
      '#default_value' => $this->getSetting('resup_settings')['resup'],
    );
...
//      '#default_value' => $this->getSetting('file_resup', 'resup_settings')['resup_max_filesize'],
...
//      '#default_value' => $this->getSetting('file_resup', 'resup_settings')['resup_autostart'],
...
//    $summary[] = t('Progress indicator: @progress_indicator', ['@progress_indicator' => $this->getSetting('progress_indicator')]);
  ...
  protected function formMultipleElements(FieldItemListInterface $items, array &$form, FormStateInterface $form_state) {
    ...
    $upload_validators = $elements['#file_resup_upload_validators'];
    ...
    $element['resup'] = array(
      ...
      '#upload_location' => $elements['#upload_location'],
      '#file_resup_upload_validators' => $upload_validators,
      '#attributes' => array(
        'class' => array('file-resup'),
        'data-upload-name' => $elements['upload']['#name'],
        'data-upload-button-name' => $elements['upload_button']['#name'],
...
//        'chunk_size' => \Drupal::config('file_resup.settings')->get('file_resup_chunksize')
      ]
    ];
    $description = $cardinality == 1 ? $element['#description'] : '';
//    $description = $cardinality == 1 ? field_filter_xss($instance['description']) : '';
...
//      '#file_resup_upload_validators' => $upload_validators,
      ...
      '#attributes' => array(
        ...
        'data-description' => $description,
//        'data-url' => url('file_resup/upload/' . implode('/', $element['#array_parents']) . '/' . $form['form_build_id']['#value']),
        'data-drop-message' => $max_files > -1 ? \Drupal::translation()->formatPlural($max_files, 'Drop a file here or click <em>Browse</em> below.', 'Drop up to @count files here or click <em>Browse</em> below.') : t('Drop files here or click <em>Browse</em> below.'),
      ),
      '#prefix' => '<div class="file-resup-wrapper">',
      '#suffix' => '</div>',
      '#attached' => array(
        'library' => array('file_resup/file_resup'),
        'drupalSettings' => $file_resup_settings,
      ),
